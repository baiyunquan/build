...
Main() {
	# Configure DNS for pbsbc01h3lite (fix for empty DNS in systemd-resolved)
	case $RELEASE in
		stretch)
			# your code here
		...
		focal)
			...
			;;
		jammy)
			# pbsbc01h3lite: Configure DNS for out-of-box connectivity
			# The default /etc/systemd/resolved.conf template has all DNS entries commented
			# This prevents DNS resolution on fresh systems
			if [[ "$BOARD" == "pbsbc01h3lite" ]]; then
				echo "Configuring DNS servers for pbsbc01h3lite..."
				cat >> /etc/systemd/resolved.conf << 'EOF'

# pbsbc01h3lite: DNS servers configured for out-of-box connectivity
# Default template had all DNS entries commented, causing resolution failure
[Resolve]
